...
class Actor(object):
    ...
    def __init__(self, observation_space, action_space, lr_scheduler=None):
        self.q = Netsoftmax(observation_space.shape[0],  configA["outputs_dim"], configA["hidden_dim"])
    ...
    # Pick an action
        observation = Variable(torch.from_numpy(observation), volatile=True).float().view(1, configA["inputs_dim"])
