import sys
import pygame
from pygame.locals import USEREVENT, QUIT, KEYDOWN, KEYUP, K_s, K_r, K_q,K_w,K_a,K_s,K_d,K_k,K_u,K_j,K_h,K_j,K_z,K_ESCAPE, K_UP, K_DOWN, K_RIGHT, K_LEFT,K_LSHIFT
...
class Simulator(object):
    ...
    def __init__(self):
        ...
        right.friction = 0.2
        top = pymunk.Segment(self.space.static_body, (0,-2), (600,-2), 1.0)
        top.friction = 0.2
    ...
    def rotation(self):
        print(str(self.leftLeg_1b_body.angle*180/np.pi)+" "+str(self.leftLeg_1a_body.angle*180/np.pi)+" "+str(self.rightLeg_1a_body.angle*180/np.pi)+" "+str(self.rightLeg_1b_body.angle*180/np.pi))
    ...
    def main(self):
       ...
       ## Object definition         
        ...
        motor_ba1Left = pymunk.SimpleMotor(self.leftLeg_1b_body, self.leftLeg_1a_body, relativeAnguVel)
        motor_ba1Left.collide_bodies = False
        
        #---link left leg a with chassis
        pj_ac1left = pymunk.PinJoint(self.leftLeg_1a_body, self.chassis_b, (legWd_a/2,0), (-chWd/2, 0))
        motor_ac1Left = pymunk.SimpleMotor(self.leftLeg_1a_body, self.chassis_b, relativeAnguVel)
        motor_ac1Left.collide_bodies = False
        ...
        motor_ba1Right = pymunk.SimpleMotor(self.rightLeg_1b_body, self.rightLeg_1a_body, relativeAnguVel)
        motor_ba1Right.collide_bodies = False
        
        #---link right leg a with chassis
        pj_ac1Right = pymunk.PinJoint(self.rightLeg_1a_body, self.chassis_b, (-legWd_a/2,0), (chWd/2, 0))
        motor_ac1Right = pymunk.SimpleMotor(self.rightLeg_1a_body, self.chassis_b, relativeAnguVel)      
        motor_ac1Right.collide_bodies = False
        ...
        # self.leftLeg_1a_shape.sensor=True
        # self.leftLeg_1b_shape.sensor=True
        # self.rightLeg_1a_shape.sensor=True
        # self.rightLeg_1b_shape.sensor=True
...
#        ---prevent collisions with ShapeFilter
        ...
        motor_ac1Left.max_force = 1000000
        motor_ac1Right.max_force = 1000000
        motor_ba1Left.max_force = 1000000
        motor_ba1Right.max_force = 1000000
        ...
        while running:
            for event in pygame.event.get():
                if event.type == QUIT or (event.type == KEYDOWN and event.key in (K_q, K_ESCAPE)):
                    ...
                    motor_ba1Right.rate = rotationRate
                elif event.type == KEYDOWN and event.key ==K_DOWN:
                    motor_ba1Right.rate = -rotationRate
                elif event.type == KEYDOWN and event.key ==K_LEFT:
                    motor_ac1Right.rate = -rotationRate
                elif event.type == KEYDOWN and event.key ==K_RIGHT:
                    motor_ac1Right.rate = rotationRate
                elif event.type == KEYUP:
                    motor_ba1Left.rate = 0
                    motor_ac1Left.rate = 0
                    motor_ba1Right.rate = 0 
                    motor_ac1Right.rate = 0
